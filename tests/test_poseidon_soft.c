...
/* ══════════════════════════════════════════════════════════════════════
 *  Poseidon-1..7 known-answer vectors (from circomlibjs via
 *  js/gen_poseidon_vectors.mjs). Inputs are [1, 2, ..., n].
 * ══════════════════════════════════════════════════════════════════════ */

static const uint8_t KAT_ALL_ARITIES[7][32] = {
    /* P1(1) */
    ...
    /* P4(1,2,3,4) */
    { 0x29,0x9c,0x86,0x7d,0xb6,0xc1,0xfd,0xd7, 0x9d,0xce,0xfa,0x40,0xe4,0x51,0x0b,0x98,
      0x37,0xe6,0x0e,0xbb,0x1c,0xe0,0x66,0x3d, 0xba,0xa5,0x25,0xdf,0x65,0x25,0x04,0x65 },
    /* P5(1,2,3,4,5) */
    { 0x0d,0xab,0x94,0x49,0xe4,0xa1,0x39,0x8a, 0x15,0x22,0x4c,0x0b,0x15,0xa4,0x9d,0x59,
      0x8b,0x21,0x74,0xd3,0x05,0xa3,0x16,0xc9, 0x18,0x12,0x5f,0x8f,0xee,0xb1,0x23,0xc0 },
    /* P6(1,2,3,4,5,6) */
    { 0x2d,0x1a,0x03,0x85,0x00,0x84,0x44,0x28, 0x13,0xc8,0xeb,0xf0,0x94,0xde,0xa4,0x75,
      0x38,0x49,0x0a,0x68,0xb0,0x5f,0x22,0x39, 0x13,0x4a,0x4c,0xca,0x2f,0x63,0x02,0xe1 },
    /* P7(1,2,3,4,5,6,7) */
    { 0x1c,0x2f,0x34,0x82,0xdb,0xb1,0x40,0xc4, 0xeb,0xb9,0xad,0xa4,0x9a,0xbd,0xbc,0x37,
      0x4a,0x9a,0x85,0xfc,0xfc,0x65,0x33,0xec, 0x2e,0x9d,0xf4,0x5b,0x49,0x21,0xc3,0x18 },
};

static void test_all_arities(void)
{
    printf("\n--- known answers for Poseidon-1..7 (circomlibjs) ---\n");

    for (size_t n = 1; n <= 7; n++) {
        uint8_t inputs[7 * 32] = {0};
        for (size_t i = 0; i < n; i++)
            u32_to_be32(inputs + 32 * i, (uint32_t)(i + 1));

        uint8_t out1[32], out2[32];
        char name[64];

        snprintf(name, sizeof(name), "P%zu hash run 1", n);
        check_ok(name, zkn_poseidon_hash(inputs, n, out1));

        snprintf(name, sizeof(name), "P%zu hash run 2", n);
        check_ok(name, zkn_poseidon_hash(inputs, n, out2));

        snprintf(name, sizeof(name), "P%zu deterministic", n);
        check_bytes(name, out1, out2, 32);

        snprintf(name, sizeof(name), "P%zu output < p", n);
        check_bool(name, out1[0] <= 0x30);

        snprintf(name, sizeof(name), "P%zu known answer (circomlibjs)", n);
        check_bytes(name, out1, KAT_ALL_ARITIES[n - 1], 32);

        printf("    P%zu(1..%zu) = 0x", n, n);
        for (int i = 0; i < 32; i++) printf("%02x", out1[i]);
        printf("\n");

        /* Sensitivity: flip one input bit → output must change. */
        uint8_t inputs_flip[7 * 32];
        memcpy(inputs_flip, inputs, sizeof(inputs_flip));
        inputs_flip[31] ^= 0x01;
        uint8_t out_flip[32];
        zkn_poseidon_hash(inputs_flip, n, out_flip);
        snprintf(name, sizeof(name), "P%zu 1-bit flip changes output", n);
        check_bool(name, memcmp(out1, out_flip, 32) != 0);
    }
}

/* ══════════════════════════════════════════════════════════════════════
 *  Reject invalid arities
 * ══════════════════════════════════════════════════════════════════════ */

static void test_invalid_arities(void)
{
    printf("\n--- invalid arity rejection ---\n");

    uint8_t inputs[8 * 32] = {0};
    uint8_t out[32];

    check_bool("reject nb_inputs=0", zkn_poseidon_hash(inputs, 0, out) != 0);
    check_bool("reject nb_inputs=8", zkn_poseidon_hash(inputs, 8, out) != 0);
}
...
int main(void)
{
    ...
    /* ── NEW: cover all arities 1..7 with circomlibjs KATs ── */
    test_all_arities();
    test_invalid_arities();
