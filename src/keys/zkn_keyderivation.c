...
cleanup:
    ...
    // Zero priv_copy BEFORE releasing the cx_bn pool lock — preserves
    // the "secret material is dead before pool state is released"
    // invariant even if a future refactor stages the private key
    // through the pool instead of the stack.
