...
void zkn_sw_sha256(const uint8_t*msg,size_t len,uint8_t out[32]){
  ...
  size_t total=len+1; while(total%64!=56) total++; total+=8;
  ...
  for(size_t o=0;o<total;o+=64){
    ...
    for(int i=0;i<64;i++){ uint32_t S1=ROR(e,6)^ROR(e,11)^ROR(e,25),ch=(e&f)^(~e&g),t1=hh+S1+ch+K[i]+w[i];
      uint32_t S0=ROR(a,2)^ROR(a,13)^ROR(a,22),maj=(a&b)^(a&c)^(b&c),t2=S0+maj;
