...
void zkn_sw_sha512(const uint8_t*msg,size_t len,uint8_t out[64]){
  ...
  size_t total=len+1; while(total%128!=112) total++; total+=16;
  uint8_t*m=calloc(total,1); memcpy(m,msg,len); m[len]=0x80;
  uint64_t bits=(uint64_t)len*8; for(int i=0;i<8;i++) m[total-1-i]=(uint8_t)(bits>>(8*i));
  for(size_t o=0;o<total;o+=128){
    uint64_t w[80];
    for(int i=0;i<16;i++){ w[i]=0; for(int j=0;j<8;j++) w[i]=(w[i]<<8)|m[o+8*i+j]; }
    for(int i=16;i<80;i++){ uint64_t s0=ROR(w[i-15],1)^ROR(w[i-15],8)^(w[i-15]>>7), s1=ROR(w[i-2],19)^ROR(w[i-2],61)^(w[i-2]>>6); w[i]=w[i-16]+s0+w[i-7]+s1; }
    uint64_t a=h[0],b=h[1],c=h[2],d=h[3],e=h[4],f=h[5],g=h[6],hh=h[7];
    for(int i=0;i<80;i++){ uint64_t S1=ROR(e,14)^ROR(e,18)^ROR(e,41),ch=(e&f)^(~e&g),t1=hh+S1+ch+K[i]+w[i];
      uint64_t S0=ROR(a,28)^ROR(a,34)^ROR(a,39),maj=(a&b)^(a&c)^(b&c),t2=S0+maj;
